function [ripples] = bz_FindRipples_ephys_tools(varargin)
...
addParameter(p,'EMGfilename',[],@isstr)
...
elseif isnumeric(varargin{1}) % if first arg is filtered LFP
    ...
    EMGfilename = p.Results.EMGfilename;
...
if strcmp(show,'on')
    ...
    elseif plotType == 2
        lfpPlot = bz_Filter(double(signal),'filter','butter','passband',[50 300],'order', 3);
