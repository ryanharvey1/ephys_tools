function [ind,tf] = ThresholdBandDetect_v2(A, lower_thresh, upper_thresh, min_sep, min_length)
...
if iscell(A)
    ...
    for i = 1:length(A)
        ind{i} = ThresholdBandDetect_v2(A{i}, lower_thresh, upper_thresh, min_sep, min_length);
    end
...
tf = zeros(length(A)+1,1); % build tf logical vector /
