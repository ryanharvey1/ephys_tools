...
for event=1:length(StartofRec)
    ...
    if isequal(linear_track,'no')
        table = table((vel_cmPerSec >=2 & vel_cmPerSec<=100),:);
    else
        table = table((vel_cmPerSec >=2 & vel_cmPerSec<=100),:);
    end
    ...
    data_video_smoothfilt(:,5)=linspace(0,(length(data_video_smoothfilt)/30),length(data_video_smoothfilt));
    ...
    if strcmp(linear_track,'yes')==1
        pix2remove=5/pixelDist; % Pix2remove=how many pixels in 8cms of track
    ...
    for i=1:length(tfile); % Counts # of .t files (# of clusters) and sets iteration length
        ...
        if PerSpkslessthan2ms>2 || sum(newCorr>0)<62 && sum(newCorr>0)>60
            [counter]=deletefiltered(tfile{i},counter,event);
            disp(['CELL ',filename, ' ISI <2ms >2%, or 60hz detected'])
            continue
        end
        ...
        [data_video_smoothfilt] = interpSpikes(table2(:,1),data_video_smoothfilt2,SpikeFile);
        spks_VEL = data_video_smoothfilt(data_video_smoothfilt(:,6) == 1,:);
        
        % filter by less than 5 spikes 
        if size(spks_VEL,1)<50 
        ...
        % Creating RateMaps for Right vs. Left trajectories
        if isequal(linear_track, 'yes')
            [SmoothRateMap_Right,SmoothRateMap_Left,DirectionalityIndex]=RightVsLeft(data_video_smoothfilt,track_length);            
        end  
...
%         data_video_smoothfilt(:,6)=linspace(0,(length(data_video_smoothfilt)/30),length(data_video_smoothfilt));
%         spks_VEL = data_video_smoothfilt(data_video_smoothfilt(:,5) == 1,:);
        ...
        % BIN & SMOOTH DATA
        [SmoothRateMap,nBinsx,nBinsy,occ,Coherence] = bindata(data_video_smoothfilt,sampleRate,spks_VEL,linear_track,track_length);
        if isequal(linear_track,'yes'); SmoothRateMapAll(i,:)=SmoothRateMap; end
        ...
        % OVERALL FIRING RATE
        OverallFR = (size(spks_VEL,1)/size(data_video_smoothfilt,1))*sampleRate;
...
%         data_video_smoothfilt(:,6)=[]; % REMOVE SPIKES FROM VIDEO DATA
        counter = counter + 1;
        close all
    end
...
%     if isequal(linear_track, 'yes') && exist('SmoothRateMap_Right','var')==1 && size(SmoothRateMap_Right,1)>1
