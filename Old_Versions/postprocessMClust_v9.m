...
for event=1:length(StartofRec)
    ...
    if isequal(linear_track,'yes');
        [X,Y] =ParameterizeToLinearTrack(table(:,2),table(:,3));
        [X2,Y2] =ParameterizeToLinearTrack(table(:,4),table(:,5));
        table=[table(:,1) X Y X2 Y2];
    end
    ...
    data_video_smoothfilt2=double([table(:,1),median([table(:,2),table(:,4)],2),median([table(:,3),table(:,5)],2),ExtractedAngle,[vel_cmPerSec;vel_cmPerSec(end,1)]]);
    ...
    for i=1:length(tfile); % Counts # of .t files (# of clusters) and sets iteration length
        ...
        if isequal(linear_track,'no')
            data_video_smoothfilt = data_video_smoothfilt((data_video_smoothfilt(:,5)>2),:);
            data_video_smoothfilt2 = data_video_smoothfilt2((data_video_smoothfilt2(:,5)>2),:);
        else
            data_video_smoothfilt = data_video_smoothfilt((data_video_smoothfilt(:,5)>2),:);
            data_video_smoothfilt3 = data_video_smoothfilt3((data_video_smoothfilt3(:,5)>2),:);
        end
        ...
        if isequal(linear_track, 'yes')
            [SmoothRateMap_Right,SmoothRateMap_Left,DirectionalityIndex]=RightVsLeft(data_video_smoothfilt3,spks_VEL,track_length2,sampleRate);
        end
        ...
        % BIN & SMOOTH DATA
        if isequal(linear_track,'yes')
            [SmoothRateMap,nBinsx,nBinsy,occ,Coherence] = bindata(data_video_smoothfilt3,sampleRate,spks_VEL,linear_track,track_length2);
        else
