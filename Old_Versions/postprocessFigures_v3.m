function p=postprocessFigures_v3(data,cellid)
com=which('postprocessFigures_v3');
...
for i=cells
    ...
    for ns=1:nsessions
        ...
        if any(contains(data.mazetypes,'CircularTrack') | contains(data.mazetypes,'LinearTrack'))
            ...
            if ns==1 % FOR LEFT RUNNING DIRECTION
                unlinearpath(ax,unlinear,data.linear_track.right{1,i}.laps,...
                    data.linear_track.right{1,i}.dataspks,data.mazetypes,...
                    data.lfp.ts,data.lfp.theta_phase(trodeID,:),c)
                
                % PLOT SPIKES ON LINEARIZED PATH WITH EACH SPIKE COLOR
                % CODED BY ITS THETA PHASE
                p(2, ns).select(); 
                ax=gca;
                spikesonpath(ax,data.linear_track.right{1,i}.dataspks,...
                    data.ratemap{i,ns},data.lfp.ts,data.lfp.theta_phase(trodeID,:),c)

                % PLOT EACH LAPS RATEMAP WITH THE OVERALL FIRING RATE
                % SUPERIMPOSED
                p(3, ns).select(); 
                ax = gca; 
                ratemaps(ax,data.linear_track.right{1,i}.maps,data.ratemap{i,ns},...
                    data.measures(i,:,ns),data.varnames)
                
                % PHASE BY POSITION 
                p(4, ns).select();
                ax = gca;
                phase_by_pos(ax,data.linear_track.right{1,i}.dataspks,...
                    data.lfp.ts,data.lfp.theta_phase(trodeID,:),...
                    data.ratemap{i,ns},data.linear_track.right{i})
                
                % PHASE MAP
                p(5, ns).select();
                ax = gca;
                phase_map(ax,data.linear_track.right{1,i}.dataspks(data.linear_track.right{1,i}.dataspks(:,6)==0,1),...
                    data.linear_track.right{1,i}.dataspks(data.linear_track.right{1,i}.dataspks(:,6)==0,2),...
                    data.linear_track.right{1,i}.dataspks(data.linear_track.right{1,i}.dataspks(:,6)==1,1),...
                    data.lfp,data.ratemap{i,ns},trodeID,data.samplerate)
                
                % PLOT AUTOCORRELATION
                p(6, ns).select(); 
                autocors(data.thetaautocorr{i,ns},data.measures(i,:,ns),data.varnames)
                
                % PLOT AVERAGE WAVEFORMS 
                p(7, ns).select(); 
                avg_waveforms(data.avgwave{i},data.measures(i,:,ns),data.varnames)
                                
            elseif ns==2 % FOR RIGHT RUNNING DIRECTION
                
                unlinearpath(ax,unlinear,data.linear_track.left{1,i}.laps,...
                    data.linear_track.left{1,i}.dataspks,data.mazetypes,...
                    data.lfp.ts,data.lfp.theta_phase(trodeID,:),c)
                ...
                p(2, ns).select();
                ax = gca;
                spikesonpath(ax,data.linear_track.left{1,i}.dataspks,...
                    data.ratemap{i,ns},data.lfp.ts,data.lfp.theta_phase(trodeID,:),c)
                ...
                p(3, ns).select(); 
                ax = gca; 
                ratemaps(ax,data.linear_track.left{1,i}.maps,data.ratemap{i,ns},...
                    data.measures(i,:,ns),data.varnames)
               
                % PHASE BY POSITION
                p(4, ns).select();
                ax = gca;
                phase_by_pos(ax,data.linear_track.left{1,i}.dataspks,...
                    data.lfp.ts,data.lfp.theta_phase(trodeID,:),...
                    data.ratemap{i,ns},data.linear_track.left{i})

                % PHASE MAP
                p(5, ns).select();
                ax = gca;
                phase_map(ax,data.linear_track.left{1,i}.dataspks(data.linear_track.left{1,i}.dataspks(:,6)==0,1),...
                    data.linear_track.left{1,i}.dataspks(data.linear_track.left{1,i}.dataspks(:,6)==0,2),...
                    data.linear_track.left{1,i}.dataspks(data.linear_track.left{1,i}.dataspks(:,6)==1,1),...
                    data.lfp,data.ratemap{i,ns},trodeID,data.samplerate)
            ...
            elseif ns>2 % FOR OPEN FIELD AND BOX
                ...
                plot_HD_tuning(data_video_spk,data.samplerate)
                
                % PLOT SPIKES ON PATH WITH EACH SPIKE COLOR
                % CODED BY ITS THETA PHASE
                p(2, ns).select(); 
                ax=gca;
                spikesonpath_2d(ax,data_video_spk,data.lfp.ts,data.lfp.theta_phase(trodeID,:),c)
                
                % PLOT RATEMAP
                p(3, ns).select(); 
                ax = gca; 
                ratemaps_2d(ax,data.ratemap{i,ns},data.measures(i,:,ns),data.varnames)
                
                % PHASE BY POSITION 
                p(4, ns).select();
                phase_by_pos_2d(data.ThPrecess{i,ns},data.ratemap{i,ns},data.measures(i,:,ns),data.varnames)
...
function unlinearpath(ax,unlinear,laps,spkframes,mazetypes,lfp_ts,theta_phase,color)
...
function spikesonpath(ax,dataspks,ratemap,lfp_ts,theta_phase,color)
% PLOT SPIKES ON LINEARIZED PATH WITH EACH SPIKE COLOR
% CODED BY ITS THETA PHASE
ts=dataspks(:,1);
x=dataspks(:,2);
...
function spikesonpath_2d(ax,dataspks,lfp_ts,theta_phase,color)
% PLOT SPIKES ON PATH WITH EACH SPIKE COLOR
% CODED BY ITS THETA PHASE
ts=dataspks(:,1);
y=dataspks(:,3);
...
function ratemaps(ax,maps,ratemap,measures,varnames)
% PLOT EACH LAPS RATEMAP WITH THE OVERALL FIRING RATE
% SUPERIMPOSED
laps=reshape([maps{:}],[],length(maps));
...
function ratemaps_2d(ax,SmoothRateMap,measures,varnames)
% ratemaps_2d 
...
if isfield(trackinfo,'fields')
    ...
    plot_corr_line(x(logical(dataspks(:,6))),length(ratemap),trackinfo)
end
end
...
hold on
plot_corr_line(ThPrecess(:,1),length(ratemap),...
...
ts=ts(I);
x=x(I);
Phasemap(ts,x,spkts,lfp,length(ratemap),trodeID,samplerate)
colormap(ax,jet(255))
end
...
function phase_colormap(ax,c,theta)
...
function plot_HD_tuning(data_video_spk,samplerate)
% PLOT TUNNING CURVE
[~,~,~,~,~,tuning]=tuningcurve(data_video_spk(data_video_spk(:,6)==0,4),...
    data_video_spk(data_video_spk(:,6)==1,4),samplerate);
angBins=0:6:360;
