...
rats={'RH11','RH13','RH14','RH16','LS17','LS19','LS21','LS23'}; % LIST RATS YOU WANT TO RUN

for irats=1:length(rats)
    parent = strcat('F:\Users\reharvey\Place_Cell_Data\PAE_Rat\',rats(irats)); % CHANGE BASED ON PARENT FOLDER
...
addpath(genpath('/Users/RyanHarvey/Dropbox/MATLAB/FMAToolbox'));
    ...
    addpath(genpath('/Users/RyanHarvey/Dropbox/MATLAB/NSMA_Toolbox'));
        ...
        rmpath(genpath('d:\Users\BClarkLab\Google Drive\MatlabDir\NSMA_Toolbox'));                rmpath(genpath('/Users/RyanHarvey/Dropbox/MATLAB/NSMA_Toolbox')); % Remove NSMA from path (different Nlynx code)
                ...
                if length(spks_VEL)<5
                    ...
                    disp(['CELL ',filename, ' LESS THAN 50 SPIKES >>>>> DELETING ASSOCIATED FILES'])
            ...
            set(0,'Showhiddenhandles','on')
                ...
                if isequal(linear_track, 'no')
                    keep('path','data_video_smoothfilt','SpikeFile','mclustpath','tfile','track_length',...
                        ...
                        ,'figures','S','sampleRate','i','StartofRec','EndofRec','event','structdir',...
                        'parent','rats','irats','timestamps','vel_cmPerSec','table2');
                elseif isequal(linear_track, 'yes')
                    keep('path','data_video_smoothfilt','SpikeFile','mclustpath','tfile','track_length',...
                        'linear_track','counter','SmoothRateMap_Right','SmoothRateMap_Left',...
                        'structdir','parent','rats','irats','Overall_DirectionalityIndex',...
                        'S','sampleRate','figures','i','StartofRec','EndofRec','event','timestamps','vel_cmPerSec','table2');
                end
                
                data_video_smoothfilt(:,5)=[]; % REMOVE SPIKES FROM VIlDEO DATA
                
                close all
                fprintf('Just finished iteration #%d of %d \n', counter,length(tfile));
                counter = counter + 1;
            end
            if exist('SmoothRateMap_Right','var')==1
                [rows,~]=size(SmoothRateMap_Right);
                if isequal(linear_track, 'yes') && rows>2
                    % NORMALIZATION
                    ...
                    keep('path','data_video_smoothfilt','SpikeFile','mclustpath','tfile','track_length',...
                        ...
                        'StartofRec','EndofRec','event','timestamps');
                    close all
                end
            end
        end
