...
clear, clc , close all
%########################################################################################################################
path=('D:\Place_Cell_Data\PAE_Rat\RH16\2016-09-28_17-00-24'); % ENTER PATH TO RAW DATA (NON-PFILE) 
...
% delete(extrastuff)
% % horizontal=line([-max(BinsNbSpikes) max(BinsNbSpikes)],[0 0]); % for running max and min
% % vertical=line([0 0],[-max(BinsNbSpikes) max(BinsNbSpikes)]);
% horizontal=line([-15 15],[0 0]);
% vertical=line([0 0],[-15 15]);
