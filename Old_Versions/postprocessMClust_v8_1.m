...
path=('D:\ClarkP30_Recordings\LB02\2016-10-07_15-26-19_SS1_1'); % ENTER PATH TO RAW DATA (NON-PFILE)
...
%     TimeFreqPlots(path,StartofRec,EndofRec,event)
    ...
    if sessionduration<11;
        error('ERROR - session timestamp error - see Data record');
    end    
    ...
    if isequal(linear_track,'no'); track_length = 76.5; end  % Arena diameter
    ...
    for i=1:length(tfile); % Counts # of .t files (# of clusters) and sets iteration length
        ...
        for j = 1:dBins;
            k = 0:dBins;
            % number of instances where the head of the rat was found in each of the 180 possible orientations
            ListOrientation = find(angle >= k(j)*2 & angle < (k(j)+1)*2);
        ...
        if isequal(linear_track, 'no')
            keep('path','data_video_smoothfilt','SpikeFile','mclustpath','tfile','track_length',...
                ...
                ,'figures','S','sampleRate','i','StartofRec','EndofRec','event','timestamps');
        elseif isequal(linear_track, 'yes')
            keep('path','data_video_smoothfilt','SpikeFile','mclustpath','tfile','track_length',...
                'linear_track','counter','SmoothRateMap_Right','SmoothRateMap_Left'...
                ,'figures','S','sampleRate','i','Overall_DirectionalityIndex','StartofRec','EndofRec','event','timestamps');
        end
