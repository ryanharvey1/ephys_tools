function [Rlength,Pval,ThetaStats]=EEGWorking_HD(eegfile,spks_VEL,StartofRec,EndofRec,event,track_length)
...
% Ryan Harvey FEB 2017, LB/RH MAR 2018
...
%     Stats.median=median;
    ...
    % PHprecession
%     ThPrecess=PHprecession(EEG_DownSampledData,EEG_DownSampledTimestamps,spks_VEL,NewSFreq,track_length,normalizedD);

%CROSS FREQUENCY COUPLING ANALYSIS BY TORT ET AL., 2010
EEGthetaData = eegfilt(EEG_DownSampledData,NewSFreq,4,12);  % THETA

% determine theta phase using the Hilbert transform
% ThetaPh = angle(hilbert(-EEGthetaData));
spks_VEL_working = interp1(EEG_DownSampledTimestamps,angle(hilbert(-EEGthetaData)), spks_VEL(:,1)', 'linear');
Rlength=circ_r(spks_VEL_working,[],[],2);
[Pval,~]=circ_rtest(spks_VEL_working);
else
