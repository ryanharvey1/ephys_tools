     ...
     if structdir(I).isdir && structdir(I).name(1) ~= '.' % IF DIR NOT '.'
        if exist(([parent filesep structdir(I).name filesep 'TT']),'file'); % LOCATE TT FOLDER
            ...
            for J=1:length(CurrentMat) % 1 TO # OF .MAT FILES IN TT FOLDER
                ...
                try
                    load(CurrentMatworking,'InformationContent','sparsity','PeakRate',...
                        'SpikeFile','spikeVec','DistanceFromTrackEnd','FieldWidth','mean_vector_length',...
                        'preferred_Direction','Direct_infoContent','DirectionalityIndex'); % OPEN .MAT WITH THESE VARS
                ...
                try
                    load(CurrentMatworking,'InformationContent','Sparsity','PeakRate',...
                        'SpikeFile','spikeVec','DistanceFromTrackEnd','FieldWidth','mean_vector_length',...
                        'preferred_Direction','Direct_infoContent','DirectionalityIndex'); % FOR DIFFERENT SPELLINGS
                    if exist('Sparsity','var')==1
                        [TTclust]= strsplit(CurrentMatworking,'_');
                        load(char(strcat(TTclust(1), '_ClusterSummary_', TTclust(2),'.mat')),'CI');
                    end
                ...
                if exist('InformationContent','var')==1 % IF INFO CONTENT EXISTS IN A GIVEN .MAT FILE, IT'S PROBABLY THE RIGHT FILE
                   ...
                   num2cell(CI.nSpikes), num2cell(CI.CluSep.IsolationDist), num2cell(CI.CluSep.L_Ratio),num2cell(mean_vector_length),num2cell(preferred_Direction),...
                   num2cell(Direct_infoContent),num2cell(DirectionalityIndex)]; % COMBINE INTO CELL ARRAY
...
AllSpikeData = cell2table(AllSpikeData,'VariableNames',{'CurentFolder','SpikeFile','InformationContent','Sparsity','PeakRate','OverallFiringRate','DistanceFromTrackEnd','FieldWidth',...
    ...
    'nSpikes','IsolationDist','L_Ratio','mean_vector_length','preferred_Direction','Direct_infoContent','DirectionalityIndex'}); % CREATE TABLE
...
disp('DONE WITH COMPILE')  
disp('NOW MAKING GRAPHS')
