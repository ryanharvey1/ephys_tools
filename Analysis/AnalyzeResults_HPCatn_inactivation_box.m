% AnalyzeResults_HPCatn_inactivation_box
clear
...
%     muscimol(fail_idx) = [];
    ...
    if irats==1
        idx(:,1)=contains(cell_id(:,1),sessions(contains(mazes,'box'))) & contains(cell_id(:,1),muscimol);
        idx(:,2)=contains(cell_id(:,1),sessions(contains(mazes,'box'))) & contains(cell_id(:,1),pbs);
        idx(:,3)=contains(cell_id(:,1),sessions(contains(mazes,'box'))) & contains(cell_id(:,1),recovery);
    elseif irats>1
        idx=[idx;[contains(cell_id(:,1),sessions(contains(mazes,'box'))) & contains(cell_id(:,1),muscimol),...
            contains(cell_id(:,1),sessions(contains(mazes,'box'))) & contains(cell_id(:,1),pbs),...
            contains(cell_id(:,1),sessions(contains(mazes,'box'))) & contains(cell_id(:,1),recovery)]];
    end
    
    clear notes mazes
end
varnames = [data.varnames,'baseline','muscimol','pbs','recovery'];
...
measures = [measures,[[ones(r,1);zeros(r,1)],...
    [zeros(r,1);idx(:,1)],...
    [zeros(r,1);idx(:,2)],...
    [idx(:,3);idx(:,3)]]];
...
writetable(Rdata,'F:\Projects\HPCatn\Rdata_hpcatn_sfn2019_inactivation_box.csv')

uniqueRowsCA(Rdata.session(contains(Rdata.condition,'drug') & contains(Rdata.drug,'muscimol')))
...
for i = 1:length(varnames)
    figure
    AllStats{i}=stat_plot(Rdata.(varnames{i})(contains(Rdata.condition,'baseline') & contains(Rdata.drug,'muscimol')),...
        Rdata.(varnames{i})(contains(Rdata.condition,'drug') & contains(Rdata.drug,'muscimol')),...
        {'baseline','muscimol'},varnames{i});
end
for i = 1:length(varnames)
    figure
    AllStatsmus_recovery{i,1}=stat_plot(Rdata.(varnames{i})(contains(Rdata.condition,'drug') & contains(Rdata.drug,'muscimol')),...
        Rdata.(varnames{i})(contains(Rdata.condition,'recovery') & contains(Rdata.drug,'muscimol')),...
        {'muscimol','recovery'},varnames{i});
end
...
for i = 1:length(session_to_test)
    load(session_to_test{i},'frames','events')
   
    
    idx = find(frames(:,1)>=events(1,1) & frames(:,1)<=events(2,1));
    idx = round(linspace(idx(1),idx(end),5));
    
    fig = figure; fig.Color = [1 1 1];
    subplot(2,4,1)
    plot(frames(idx(1):idx(2),2),frames(idx(1):idx(2),3),'.k')
    axis image
    title(session_to_test{i})
    subplot(2,4,2)
    plot(frames(idx(2):idx(3),2),frames(idx(2):idx(3),3),'.k')
    axis image
    subplot(2,4,3)
    plot(frames(idx(3):idx(4),2),frames(idx(3):idx(4),3),'.k')
    axis image
    subplot(2,4,4)
    plot(frames(idx(4):idx(5),2),frames(idx(4):idx(5),3),'.k')
    axis image
    
    
    
    idx = find(frames(:,1)>=events(1,2) & frames(:,1)<=events(2,2));
    idx = round(linspace(idx(1),idx(end),5));
...
%     set(gcf, 'Units', 'Normalized', 'OuterPosition', [3 309 1675 724]);
