...
for i=1:length(VarNames)
    groups=[repmat({'Group1'},length(Group1),1);repmat({'Group2'},length(Group2),1)];
%     groups=data.textdata.Session1; groups(1,:)=[]; groups=groups(~cellfun(@isempty,groups));
    ...
    disp('Sphericity')
    mauchly(rm2)
    disp(['Homogenity of Variance, p: ',num2str(vartestn(measureVal(:,1:3),'TestType','LeveneQuadratic','Display','off'))])
    ...
    disp('Sphericity')
    mauchly(rm3)
    disp(['Homogenity of Variance, p: ',num2str(vartestn(measureVal(:,3:5),'TestType','LeveneQuadratic','Display','off'))])
    ...
    disp('Sphericity')
    mauchly(rm4)
    disp(['Homogenity of Variance, p: ',num2str(vartestn(measureVal(:,[1,3,5]),'TestType','LeveneQuadratic','Display','off'))])
