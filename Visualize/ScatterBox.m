function [ AllStats ] = ScatterBox(Group1,Group2,GroupNames,VarNames,plots)
...
%       notBoxPlot.m can be found here: http://www.mathworks.com/matlabcentral/fileexchange/26508-notboxplot
...
for vars=1:size(VarNames,2)
    % Remove NaNs
    con=Group1(:,vars); con(isnan(con))=[]; PAE=Group2(:,vars); PAE(isnan(PAE))=[];
...
if plots~=3
    ...
    for vars=1:size(VarNames,2)
        if hypothesis1(vars)==1 && plots==1 || plots==2
            ...
            set(h(1).semPtch,'Visible','off')
            set(h(1).mu,'Visible','on')
            set(h(1).mu,'LineWidth',10)
            set(h(1).mu,'XData',h(1).med.XData+[-.15 .15])
            set(h(1).sd,'Visible','off')
            set(h(1).data,'markerfacecolor', [0.5 0.5 0.5])
            set(h(1).data,'markersize',10)
            set(h(1).med,'XData',h(1).med.XData+[-.15 .15])
            set(h(1).med,'Color',[1 .5 0])
            set(h(1).med,'LineStyle','-')
            set(h(1).med,'LineWidth',10)
            
            set(h(2).semPtch,'Visible','off')
            set(h(2).mu,'Visible','on')
            set(h(2).mu,'XData',h(2).med.XData+[-.15 .15])
            set(h(2).mu,'LineWidth',10)
            set(h(2).sd,'Visible','off')
            set(h(2).data,'markerfacecolor', [0.5 0.5 0.5])
            set(h(2).data,'markersize',10)
            set(h(2).med,'XData',h(2).med.XData+[-.15 .15])
            set(h(2).med,'Color',[1 .5 0])
            set(h(2).med,'LineStyle','-')
            set(h(2).med,'LineWidth',10)
%             title(VarNames(vars),'FontSize',14);
            set(gca,'XTickLabel',GroupNames,'FontSize',font)
            set(gca,'XTickLabel',GroupNames,'FontWeight','bold')
            ylabel(VarNames(vars),'FontSize',font);
            set(gca,'box','off')
