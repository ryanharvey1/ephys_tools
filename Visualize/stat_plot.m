...
for vars=1:size(VarNames,2)
    ...
    % Concat Stats
    if Kolmogorov>=1
        AllStats(vars,:)=strcat(cellstr(VarNames(vars)),': Z=',num2str(round(statz.zval,2)),', p=',num2str(p),', d=',num2str(round(cod,2)),...
            ',','',GroupNames(1),' Median=',num2str(round(median1,2)),', ',GroupNames(2),' Median=',num2str(round(median2,2)));
    else
        AllStats(vars,:)=strcat(cellstr(VarNames(vars)),': t(',num2str(round(statz.df)),')','=',num2str(round(statz.tstat,2)),', p=',num2str(p),', CI[',[num2str(round(ci(1),2)),',',num2str(round(ci(2),2))],']',...
