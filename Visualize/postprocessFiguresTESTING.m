...
classdef postprocessFiguresTESTING
    
    methods(Static)
        function p=main(data,cellid)
            ...
            if any(any(contains(data.mazetypes,'CircularTrack') | contains(data.mazetypes,'LinearTrack')))
                ...
                location = reshape(1:8*nsessions, nsessions, 8)';
            ...
            for i=cells
                ...
                fig.Color=[1 1 1];
                ha = tight_subplot(8,nsessions,[.05 .01],[.04 .04],[.01 .01]);
                
                
                for ns=1:nsessions
                    axes(ha(location(1,ns)));
                    ax=gca;
                    
                    if any(contains(data.mazetypes,'CircularTrack') | contains(data.mazetypes,'LinearTrack'))
                        
                        if ns==1 % FOR LEFT RUNNING DIRECTION
                            postprocessFigures.unlinearpath(ax,unlinear,data.linear_track.right{1,i}.laps,...
                            ...
                            axes(ha(location(2,ns)));
                            ax=gca;
                            postprocessFigures.spikesonpath(ax,data.linear_track.right{1,i}.dataspks,...
                                data.ratemap{i,ns},data.lfp.ts,data.lfp.theta_phase(trodeID,:))
                            
                            % PLOT EACH LAPS RATEMAP WITH THE OVERALL FIRING RATE
                            % SUPERIMPOSED
                            axes(ha(location(3,ns)));
                            ax = gca;
                            postprocessFigures.ratemaps(ax,data.linear_track.right{1,i}.maps,data.ratemap{i,ns},...
                                data.measures(i,:,ns),data.varnames)
                            
                            % PHASE BY POSITION
                            axes(ha(location(4,ns)));
                            ax = gca;
                            postprocessFigures.phase_by_pos(ax,data.linear_track.right{1,i}.dataspks,...
                                data.lfp.ts,data.lfp.theta_phase(trodeID,:),...
                                data.ratemap{i,ns},data.linear_track.right{i})
                            
                            % PHASE MAP
                            axes(ha(location(5,ns)));
                            ax = gca;
                            postprocessFigures.phase_map(ax,data.linear_track.right{1,i}.dataspks(data.linear_track.right{1,i}.dataspks(:,6)==0,1),...
                        ...
                        elseif ns==2 % FOR RIGHT RUNNING DIRECTION
                            ...
                            axes(ha(location(2,ns)));
                            ax = gca;
                            postprocessFigures.spikesonpath(ax,data.linear_track.left{1,i}.dataspks,...
                                data.ratemap{i,ns},data.lfp.ts,data.lfp.theta_phase(trodeID,:))
                            ...
                            axes(ha(location(3,ns)));
                            ax = gca;
                            postprocessFigures.ratemaps(ax,data.linear_track.left{1,i}.maps,data.ratemap{i,ns},...
                                data.measures(i,:,ns),data.varnames)
                            
                            % PHASE BY POSITION
                            axes(ha(location(4,ns)));
                            ax = gca;
                            postprocessFigures.phase_by_pos(ax,data.linear_track.left{1,i}.dataspks,...
                                data.lfp.ts,data.lfp.theta_phase(trodeID,:),...
                            ...
                            axes(ha(location(5,ns)));
                            ax = gca;
                            postprocessFigures.phase_map(ax,data.linear_track.left{1,i}.dataspks(data.linear_track.left{1,i}.dataspks(:,6)==0,1),...
...
%                 subplot('Position',[0.01 0.01 .98 .1])
%                 plotSpikeRaster(data.Spikes(i),'PlotType','vertline');hold on
%                 for ii=1:size(data.events,2)
%                     plot([data.events(1,ii),data.events(1,ii)],ylim,'b','LineWidth',3)
%                     plot([data.events(2,ii),data.events(2,ii)],ylim,'r','LineWidth',3)
%                 end
%                 axis off
            end
