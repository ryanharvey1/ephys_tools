function wf = getWaveForms_filtered(gwfparams)
...
% Edits by Ryan Harvey to handle filtered.dat files (2019)
%
% % EXAMPLE INPUT
...
mmf = memmapfile(fileName, 'Format', {gwfparams.dataType, [nSamp gwfparams.nCh ], 'x'});
% remove a bit of overhead
x=mmf.Data.x;
...
waveForms = nan(numUnits,gwfparams.nWf,wfNSamples,nChInMap);
waveFormsMean = nan(numUnits,wfNSamples,nChInMap);
for curUnitInd=1:numUnits
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        tmpWf = x(spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(1):spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end),1:gwfparams.nCh);
...
wf.waveForms = permute(waveForms,[1 2 4 3]);
wf.waveFormsMean = permute(waveFormsMean,[1 3 2]);
