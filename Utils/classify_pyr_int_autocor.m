...
df_cells = readtable('/Users/lauraberkowitz/Documents/drift_analysis/shuffled_mlv.csv');
data_path = '/Users/lauraberkowitz/Documents/ProcessedData/';
save_path = '/Users/lauraberkowitz/Documents/cell_classification/';
df_cells = df_cells(df_cells.shuff_pass == 1,:);
% keep cells that passed mvl 
df_cells.session = df_cells.SessionID;

%% run through each session
WaitMessage = parfor_wait(length(unique(df_cells.SessionID)),'Waitbar',false,'ReportInterval',1);
sessions = unique(df_cells.SessionID);
...
% eva.OptimalK
[idx,C] = kmeans(score(:,1:2),4,'Replicates',20,'Distance','sqeuclidean');
...
% get autocor 
autocor = get_autocor(data.Spikes);
...
function diff_hist_normalize = get_autocor(spikes)
edges = 0:.0005:.012;
diff_hist_normalize = zeros(length(spikes),length(edges)-1);
for i = 1:size(spikes,1)
    diff_hist = histcounts(diff(spikes{i}),edges);
    diff_hist_normalize(i,:) = diff_hist/sum(diff_hist);
end
