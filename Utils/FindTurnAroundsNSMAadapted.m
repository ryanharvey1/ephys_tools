function [startgoodrun, stopgoodrun]=FindTurnAroundsNSMAadapted(data_video_nospk,laps,runthresh)
...
MinGoodDirectionTime = 15;     % minimum time to be considered a good running period (1 sec, in ts units)
MinBadDirectionTime = 30;   % minimum time to be considered a break in running (1 sec, in ts units)

framesPerSec=30;            % tracker sampling rate

% smooth position data
% sV=SmoothTsd(V,framesPerSec);
sV=smooth(data_video_nospk(:,2),framesPerSec);
% find velocity of rat from track position
% vel_v = diff(Data(sV));			% vel units are pixels/frame
vel_v = diff(sV);
% vel_t = Range(V,'ts');
vel_t=data_video_nospk(:,1);
...
% figure; plot(data_video_nospk(:,1), data_video_nospk(:,2), 'k-')
% xlabel('Time (ts from start of cheetah)')
% ylabel('Position on Track')
% axis tight
% hold on; plot(wrongdir_t, interp1(data_video_nospk(:,1), data_video_nospk(:,2), wrongdir_t), 'r.')
% hold on; scatter(S, interp1(data_video_nospk(:,1), data_video_nospk(:,2), S), 'g.')
% hold on; scatter(E, interp1(data_video_nospk(:,1), data_video_nospk(:,2), E), 'b.')
...
% figure;plot(data_video_nospk(~ismember(data_video_nospk(:,1),wrongdir_t),1),data_video_nospk(~ismember(data_video_nospk(:,1),wrongdir_t),2),'.k')
