...
% chMap = readNPY(fullfile(gwfparams.myKsDir, 'channel_map.npy'))+1;               % Order in which data was streamed to disk; must be 1-indexed for Matlab
% nChInMap = numel(chMap);
...
for curUnitInd=1:numUnits
    ...
    % remove spikes that occur to close to the end of the session
    spikeTimesRP(spikeTimesRP+gwfparams.wfWin(end)>size(mmf.Data.x,2))=[];
    spikeTimeKeeps(curUnitInd,1:min([gwfparams.nWf curUnitnSpikes])) =...
        sort(spikeTimesRP(1:min([gwfparams.nWf curUnitnSpikes])));
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        tmpWf = mmf.Data.x(1:gwfparams.nCh,spikeTimeKeeps(curUnitInd,curSpikeTime)+...
            gwfparams.wfWin(1):spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end));
