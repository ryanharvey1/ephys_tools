...
if nargin < 2 | mod(length(varargin),2) ~= 0,
  error('Incorrect number of parameters (type ''help <a href="matlab:help PlotDistribution">PlotDistribution</a>2'' for details).');
end

if isa(var1,'cell'),
	if ~isa(var2,'cell'),
		error('One parameter is a cell array and the other is not (type ''help <a href="matlab:help PlotDistribution">PlotDistribution</a>2'' for details).');
	end
	if length(var1) ~= length(var2),
		error('Variable lists have different lengths (type ''help <a href="matlab:help PlotDistribution">PlotDistribution</a>2'' for details).');
	end
else
	var1 = {var1};
	var2 = {var2};
...
for i = 1:length(var1),
	if ~isdvector(var1{i}) | ~isdvector(var2{i}) | any(size(var1{i})~=size(var2{i})),
		error('Individual variables must be vectors of identical length (type ''help <a href="matlab:help PlotDistribution">PlotDistribution</a>2'' for details).');
	end
end
...
for i = 1:2:length(varargin),
  if ~ischar(varargin{i}),
    error(['Parameter ' num2str(i+2) ' is not a property (type ''help <a href="matlab:help PlotDistribution">PlotDistribution</a>2'' for details).']);
  end
  switch(lower(varargin{i})),
    case 'nbins',
      nBins = varargin{i+1};
      if ~isiscalar(nBins,'>0'),
        error('Incorrect value for property ''nBins'' (type ''help <a href="matlab:help PlotDistribution">PlotDistribution</a>2'' for details).');
      end
    case 'smooth',
      smooth = varargin{i+1};
      if ~isdscalar(smooth,'>=0'),
        error('Incorrect value for property ''smooth'' (type ''help <a href="matlab:help PlotDistribution">PlotDistribution</a>2'' for details).');
      end
    otherwise,
      error(['Unknown property ''' num2str(varargin{i}) ''' (type ''help <a href="matlab:help PlotDistribution">PlotDistribution</a>2'' for details).']);
  end
end
