...
if size(positions,2) ~= 3,
  warning('Parameter ''positions'' is not a Nx3 matrix - using center of mass of all LEDs.');
  positions(:,2) = mean(positions(:,2:2:end),2);
  positions(:,3) = mean(positions(:,3:2:end),2);
end
