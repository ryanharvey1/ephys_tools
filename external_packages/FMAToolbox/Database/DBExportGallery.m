...
function DBExportGallery(query,galleryName,varargin)
...
try
	mkdir(galleryName);
	mkdir([galleryName '/images']);
	mkdir([galleryName '/thumbs']);
	if showCode,
		mkdir([galleryName '/code']);
	end
catch
	error('Cannot create gallery (check file access permissions).');
end
[path,galleryName] = fileparts(galleryName);
...
for i = 1:nFigures,
	...
	file = fopen([path galleryName '/images/' figureName{i}],'wb');
	if file == -1,
	...
	% Thumbnail
	pngData = imread([path galleryName '/images/' figureName{i}],'png');
   ...
   imwrite(thumbnail,[path galleryName '/thumbs/' figureName{i}],'png');

	% Code
	if showCode && ~isempty(f.code{i}),
		file = fopen([path galleryName '/code/' figureName{i}(1:end-4) '.m'],'w');
		if file == -1,
...
% Output HTML code
% Header...
fid = fopen([path galleryName '/index.html'],'w');
