...
%     'maxGap'      time gaps between successive samples exceeding this
%                   threshold will not be interpolated (default = Inf)
%    =========================================================================
%
%  OUTPUT
%
...
if isempty(timestamps) | isempty(samples), interpolated = []; return; end
...
large = [];
if ~isinf(maxGap),
	gaps = [0;diff(samples(:,1))];
	large = find(gaps>maxGap);
	if ~isempty(large),
		start = samples(large-1,1);
		stop = samples(large,1);
	end
end
...
if pointProcess,
	...
	keep = logical(ones(size(timestamps)));
	discarded = logical(zeros(size(timestamps)));

	% Discard timestamps that would require extrapolation
	if strcmp(trim,'on'),
		keep = timestamps>=samples(1,1)&timestamps<=samples(end,1);
		discarded = ~keep;
	...
	% Discard values interpolated in large time gaps
	if ~isempty(large),
		discarded(keep) = InIntervals(interpolated,[start stop]);
		interpolated(discarded(keep),:) = [];
	end
end