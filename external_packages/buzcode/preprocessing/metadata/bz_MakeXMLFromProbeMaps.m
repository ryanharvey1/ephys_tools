...
s = re_color_spike_groups(s,groupchans_byprobe);
...
function s = re_color_spike_groups(s,groupchans_byprobe,varargin)

p = inputParser;
p.addParameter('by_spikegroup',1);
p.addParameter('color_map','hsv');
p.parse(varargin{:});

by_spikegroup = p.Results.by_spikegroup;
color_map = p.Results.color_map;

n_groups = length(groupchans_byprobe);
hex_colors = cellstr(rgb2hex(feval(color_map,n_groups)));
% hex_colors = cellstr(rgb2hex([rand(n_groups,1),rand(n_groups,1),rand(n_groups,1)]));

loc_color = find(contains(s,'spikeColor'));

ii = 1;
for i = 1:length(hex_colors)
    for tt = 1:length(groupchans_byprobe{i})
        s(loc_color(ii)) = {['  <spikeColor>',hex_colors{i},'</spikeColor>']};
        ii = ii + 1;
    end
end
end
