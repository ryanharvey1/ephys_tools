...
[speedgrid,speedVec,speed] = speed_map(posx_c,posy_c,n_speed_bins,sampleRate);

% compute theta matrix
[thetagrid,thetaVec,phase] = theta_map(filt_eeg,post,eeg_sample_rate,n_theta_bins);

% remove times when the animal ran > 60 cm/s (these data points may contain artifacts)
too_fast = find(speed >= 60);
