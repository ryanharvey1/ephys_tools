...
%
%  [avgEEG, errEEG, EEGts, EEGsamples] = PETH_EEG( T, eeg_fname, tlow_msec, thigh_msec)
%
...
%
%  PL 11/08/2001
%  updated to new mex function ReadCR  PL 08/13/2009
...
for i = 1:nT
    [eeg_ts{i}, eeg{i}, sFreq] = ReadCR( eeg_fname, T(i)-abs(tlow_msec)*10, T(i)+abs(thigh_msec)*10);
...
% means and errors
avgEEG = mean(EEGsamples);
errEEG = std(EEGsamples)/sqrt(nT);