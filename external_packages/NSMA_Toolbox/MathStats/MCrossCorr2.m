function [C,B,CC,db] = MCrossCorr2(t1,t2,dx,nbins)
% Modification of Francescos original C CrossCorr:
%   1) Reversed roles of t1 and t2: 
%       In Francesco's original version the CrossCorr function is
...
%       the RIGHT of histogram center (with peak center at +DT). This is
%       the opposite of the result obtained with matlabs xcorr on binned
%       spiketrains. The corrent version 2.0 now uses matlabs convention
%       where the roles of t1 and t2 are reversed and C corresponds to a
%       PETH of spiketrain t1 around events given by t2. The peak in above
%       example now appears to the LEFT of the histogram center (with peak
%       center at -DT).
%   2) Inclusion of the lower bin edges:
%       
%       
%
