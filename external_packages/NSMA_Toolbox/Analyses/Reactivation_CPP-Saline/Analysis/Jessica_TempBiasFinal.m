...
CPP = 1    % set to 0 for saline, 1 for CPP

if ~CPP
    run_label = 'Saline';
    indir = 'D:\Data\4jason\jason\Out\TempBias_Saline_15min';
    ratlist = {'6318_200ms';'6401_200ms';'6495_200ms';'6496_200ms';'6568_200ms';'6591_200ms'};
else    
    run_label = 'CPP';
    indir = 'T:\CPP1\JessicasData\Analysis\out\TempBias_CPP_15min';
    ratlist = {'7508_200ms';'7651_200ms'};
end


outputDir = indir
outPrefix = 'RegAnalysis_';
...
for jj = 1:length(ratlist)  
    ...
    filename = [ratlist{jj} '_bias_results.mat'];
    load(filename);
    fprintf('\n\n=========================== %s ============================\n', filename);
    
    X = [ones(length(ABM),1) ABM(:,2)];     % ABM is the "all bias matrix which has three columns representing all the bias values for  % S1 - B - S2 respectively
    ...
    title([ratlist{jj} ': B vs S1    (' num2str(NDots) ' data points)'])
    xlabel('Behavior')
    ylabel('Sleep 1')
    axis([-1,1,-1,1])
    axis equal
    ...
    plot(ABM(:,2),ABM(:,3),'.') 
    grid on
    title([ratlist{jj} ': B vs S2    (' num2str(NDots) ' data points)'])
    xlabel('Behavior')
    ylabel('Sleep 2')
    axis([-1,1,-1,1])
    axis equal
    ...
    s1file = [ratlist{jj} '_S1Bstats'];
    s2file = [ratlist{jj} '_S2Bstats'];
...
X = [ones(length(B_bias_pooled),1) B_bias_pooled];
...
grid on
title([run_label ' ' num2str(length(ratlist)) 'Rats combined: B vs S1    (' num2str(NDots) ' data points)'])
xlabel('Behavior')
ylabel('Sleep 1')
axis([-1,1,-1,1])
axis equal
...
grid on
title([run_label ' ' num2str(length(ratlist)) ' Rats combined: B vs S2    (' num2str(NDots) ' data points)'])
xlabel('Behavior')
ylabel('Sleep 2')
axis([-1,1,-1,1])
axis equal
