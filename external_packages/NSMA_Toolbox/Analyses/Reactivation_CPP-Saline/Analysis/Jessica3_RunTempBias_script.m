% RunTempBias script 3
% modified from script 2 to compute TempBias EVs for each sessions AND all sessions per rat combined
%
% 
...
% write CSV headers
fprintf(fpCSV1,['Rat,','Session,','Group,','Length of Sleep 1 (sec),', 'length of Behavior (sec),', 'length of Sleep 2 (sec),','# Cells before cuts,',...
        '# Cells after cuts,','# Pairs before cuts,', '# Pairs after NaNs and 1s removal,',...
        '# Pairs after area cut,', 'Partial Corr,','Rev. Partial Corr','\n']);
fprintf(fpCSV2,['Rat,','Group,','# Sessions,','# Cells per rat,','# Pairs before cuts,',...
        '# Pairs after NaNs and 1s removal,', '# Pairs after area cut,', 'Partial Corr,','Rev. Partial Corr','\n']);
...
for irat = 1:length(ratlist)
    ...
    for isess = 1:length(working)
        ...
        % script 3 TEMPBIAS EV per session
        ...
        [biasS1sess, preS1sess, postS1sess] = getCCGBias(CCGS1{irat,isess}, norm, width);
        [biasBsess,  preBsess,  postBsess]  = getCCGBias(CCGB{irat,isess},  norm, width);
        [biasS2sess, preS2sess, postS2sess] = getCCGBias(CCGS2{irat,isess}, norm, width);
        NPairsPerSessPreCuts = length(biasS1sess);
        
        % Make a Bias-Matrix with each row: [biasS1, biasB, biasS2, biasS1-biasS1] and one row per pair of cells 
        ABM = [biasS1sess biasBsess biasS2sess];
        ABM(:,4) = (ABM(:,3) - ABM(:,1));                   % Sleep 2 - Sleep 1 
        preM = [preS1sess preBsess preS2sess];
        postM = [postS1sess postBsess postS2sess];
        ...
        NPairsPerSessPostAreaCut = nr;
                
        % for Temp Bias EV per Sessions 
        S1B_sessbiascorr = diag(corrcoef(ABM(:,1),ABM(:,2)),1);
        S2B_sessbiascorr = diag(corrcoef(ABM(:,3),ABM(:,2)),1);
        S2B_sessbiassub = diag(corrcoef(ABM(:,4),ABM(:,2)),1);
        S1S2_sessbiascorr = diag(corrcoef(ABM(:,1),ABM(:,3)),1);
        
        
        %Partial Correlation
        norm = sqrt((1 - S1B_sessbiascorr^2) * (1 - S1S2_sessbiascorr^2));    
        S2B_sessbiaspart{irat,isess} = (S2B_sessbiascorr - S1B_sessbiascorr*S1S2_sessbiascorr)/ norm;
        % compute the reversed partial corrleation = sqrt( explained variance (EV)) with role of S1 and S2 interchanged for m,s2|s1
        norm_rev = sqrt((1 - S2B_sessbiascorr^2) * (1 - S1S2_sessbiascorr^2));    
        S2B_sessbiaspart_rev{irat,isess} = (S1B_sessbiascorr - S2B_sessbiascorr*S1S2_sessbiascorr)/ norm_rev;
        
        fprintf('\n RAT %s, session %d, sessid = %s,  %d cells:\n EV = %f, EV_rev = %f \n',...
            ses.animal, nsess, ses.name, N, S2B_sessbiaspart{irat,isess}(1), S2B_sessbiaspart_rev{irat,isess}(1));
        
        % save session info to CVS
        fprintf(fpCSV1,['%s,','%s,','%s,','%f,','%f,','%f,','%d,','%d,','%d,','%d,','%d,','%f,','%f','\n'],...
            ses.animal,ses.name,group,SS1_interval_sec,SB_interval_sec,SS2_interval_sec,NCellsPreCuts, N ,...
            NPairsPerSessPreCuts,NPairsPerSessPostNaNsCut, NPairsPerSessPostAreaCut, ...
            S2B_sessbiaspart{irat,isess}(1), S2B_sessbiaspart_rev{irat,isess}(1) );
    ...
    save(filename,'bias*','pre*','post*','CCG*','*allbia*','ABM','NCellsPerRat','animalSessionList','nsess','*sessbiaspart*');
