...
* cowen Feb 2002 - modified to load old v1.? UNIX cheetah data.
 . Jean-Marc & Peter 2008 - corrected start_pos to find record that CONTAINS the start timestamp and not the record follwing the one with the start ts.
--------------------------------*/
					  ...
					  int nINP, const mxArray *pINP[])
{
	...
	while ( i < nSamples )
	{
		...
		if (!found_start && blk.TimeStamp >= start_ts)
		{
			...
			found_start = 1;
			start_pos = ftell(fp)-NEW_CR_RECSIZE;   // -sizof(blk) corrected by Jean-Marc & Peter to get the block CONTAINING the current timesamps (May 2008)
		}
		...
		if (i<1) {
			...
			/* mexPrintf("junk[%d] = %d\n", i,junk);  */
			mexPrintf("Sampling Frequency in %s is %f \n", 
				fn,sampFreq);  
			}
    ...
    fseek(fp, start_pos, SEEK_SET);    // read from record containing the start timestamp
	...
	for (i = 0; i < nSamples; i++) {
		...
		//mexPrintf(" 4Timestamp %g \n",blk.TimeStamp );		
	
		t[i] = blk.TimeStamp;

		for (j = 0; j<512; j++)
			if(j < blk.NumValidSamples)
				cr[j + 512 * i] = blk.Data[j];
			else
				cr[j + 512 * i] = 0.0;
	} 
