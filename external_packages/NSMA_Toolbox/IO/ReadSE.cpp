...
void ReadTT(char* fn, int nSpikes, double *t, double *wv){
	...
	if (new_NT_format){ 
		...
		for (int i = 0; i < nSpikes; i++){
			...
			for(int k = 0; k<32; k++)
				wv[i +  nSpikes*4*k] = (double) swapbytes(snData[k]);  
				...
				t[i] = (double) qwTimeStamp/100.0;
			for(int k = 0; k<32; k++)
				wv[i +  nSpikes*4*k] = (double) snData[k];  
			}
...
void ReadTTByRecord(char* fn, double *records_to_get, int n_records_to_get, double *t, double *wv){
	...
	{ 
		...
		while(i < n_records_to_get)
		{
			...
			///////////////////////////////////////////////////////////////////
			fseek(fp, postHeaderPos+sizeof(char)*(8+40+64)*((long)records_to_get[i] - 1), SEEK_SET);
			...
			if(bigendianFlag)
			{
				...
				t[i] = (double) qwTimeStamp/100.0;
			    for(int k = 0; k<32; k++)
				    wv[i +  n_records_to_get*4*k] = (double) swapbytes(snData[k]);  
			} 
			else 
			{
				///////////////////////////////////////////////////////////////////
				...
				t[i] = (double) qwTimeStamp/100.0;
			    for(int k = 0; k<32; k++)
				    wv[i +  n_records_to_get*4*k] = (double) snData[k];  
			}
			i++;
		}
...
void ReadTT_timestamps(char* fn, int nSpikes, double *t){
	...
	if (new_NT_format){ 
		...
		for (int i = 0; i < nSpikes; i++){
			...
			fread(&qwTimeStamp0,  sizeof(char),   8, fp);
			fseek(fp, sizeof(char)*(40+64), SEEK_CUR);
