...
%   $Revision: 1.1.6.4 $  $Date: 2011/05/09 00:40:38 $ 
...
case 'B-'
  ...
  {'';['The B(asic)-SPLINE with knot sequence  t(i) < ... < t(i+k)  is',...
   ...
   ['In Curve Fitting Toolbox, the B-splines are normalized so that they',...
   ' sum to 1,'];'';
...
case 'qu'
   ...
   ' of order 4.'];'';
   ['In Curve Fitting Toolbox, the quintic smoothing spline is provided by',...
...
case 'si'
  ...
  {['The terms  SITES  and  VALUES  are used here instead of their',...
   ' old fashioned counterparts,  ABSCISSAE  and  ORDINATES.'];'';
...
if nargout>0 
   ...
   msgbox(mess, sprintf( 'Explanation: %s', term ))
end
