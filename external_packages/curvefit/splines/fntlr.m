...
%   $Revision: 1.1.6.3 $  $Date: 2010/10/08 16:38:05 $
...
if nargin>3  % check that both X and INTERV are of the correct size
   ...
   if length(interv)~=2
      error(message('SPLINES:FNTLR:notenoughinterv', num2str( m )))
   end
end
...
if form(1)=='r'  % we now have to convert this all to the values of the
   ...
   if length(dorder)>2
      warning(message('SPLINES:FNTLR:onlyonortw'))
      return
   end

   if m>1 % x is expected to be a matrix, of size [m,lx]
      if iscell(x), error(message('SPLINES:FNTLR:nogrid')), end
