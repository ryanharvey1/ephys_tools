...
%   $Revision: 1.1.6.9.2.1 $  $Date: 2011/07/18 00:31:18 $
...
switch action
    ...
    case 'export_data'
        ...
        xname = ['x',curtag]; yname = ['y',curtag]; accepted = 0;
        while ~accepted
            answer = inputdlg({...
            ...
            if ~isempty(answer)
                ...
                if isequal(xname,answer{1}) % increment the counter kept in tag
                    set(hand.dataline,'Tag', num2str(eval([curtag,'+1'])))
                end
        ...
        if accepted
            ...
            fprintf( 'Splinetool GUI created the variables %s, %s in the current workspace.\n', ...
                xname, yname )
    ...
    case 'export_spline'
        ...
        fprintf( 'Splinetool GUI created the variable %s in the current workspace.\n', ...
            answer{1} )
    ...
    case 'help'
        titlestring = sprintf( 'Explanation: %s',y );
        switch y
            ...
            case 'about the GUI'
                mess =...
                    ...
                    ['The ''Bottomline'' shows the command that generated',...
            ...
            case 'quintic smoothing spline'
                mess = {...
                    ...
                    ' of order 4.'];'';...
                    ['The quintic smoothing spline is provided by the command  spaps . The ', ...
                    ' level of desired smoothness is specified indirectly, by specifying a', ...
                    ' certain tolerance, and the command returns the smoothest spline', ...
                    ' within that tolerance of the given data.']};
    ...
    case 'highlightxy' % highlight the marked item
        markxy(hand)
    ...
    case 'pm'  % one of the increment/decrement guys has been clicked
        ...
        j = find('telmr'==tag(end));
        if j==1 && get(hand.method,'Value')==2  % allow for special representation
            % of smoothing parameter near 1
            set(hand.params(2,1),'String', shortstr(eval([get(hand.params(2,1), ...
                'String'), pm, get(hand.params(4,1),'String')]),'',1));
        else
            set(hand.params(2,j),'String', eval([get(hand.params(2,j),'String'),...
                pm, get(hand.params(4,j),'String')]));
        end
    ...
    case 'print_graph'  % preserve current graphs as separate figure
        ...
        printfig = figure(...
            ...
            'NumberTitle','off',  ...
            'HandleVisibility','on', ...
            'Name',sprintf( 'Spline Tool graph (at %s)', showtime ) );
    ...
    case 'save2mfile'
        ...
        getfiletitle = 'Choose a Name for the File';
                ...
                if isvarname(mfname) % if a valid name, check whether it is taken
                    ...
                    if ~exist(fullfilename, 'file')
                        ...
                        if checked
                            ...
                            anss = questdlg( ...
                                sprintf( '%s already exists.  Do you want to replace it?', which( fullfilename ) ), ...
                                'File already exists ...', 'No','Yes','No');
                        end
                        if isequal(anss,'Yes')
                            filenameChosen = true;
                        end
                    ...
                    temp = errordlg( sprintf( 'The filename, %s, you chose is not valid.', mfname ), ...
                        'Try again ...' );
                    getfiletitle = 'Choose  V A L I D  MATLAB Name for the File';
                    waitfor(temp)
                end
            end
        end
        ...
        if datachanged||othermanual
            fprintf(mfid,['%%\n',...
                ...
                '%%   sites other than the ones used in SPLINETOOL when this file\n',...
                '%%   was written.\n']);
        end
        % append the help
        ...
        for v=vv
            ...
            switch lineud.method
                ...
                case 3
                    ...
                    if isfield(lineud,'kc')&&~isempty(lineud.kc), setknots = 1; end
                case 4
                    if isfield(lineud,'kc')&&~isempty(lineud.kc), setknots = 1; end
            end
            ic = [];
        ...
        fclose(mfid);
        fprintf( 'SplineTool generated code in the file %s in the directory %s\n', mfname, pn(1:end-1) );
    ...
    case 'start'
        ...
        hand.figure = figure(...
            ...
            'MenuBar','none', ...
            'ToolBar','none');
        % If we are in "testing mode" then allow greater visibility of the figure
        % window
        if ~testing
            set( hand.figure, 'HandleVisibility', 'callback' )
        end
        ...
        try
            ...
            switch y
                ...
                case 7 % move knots to improve an interpolant
                    ...
                    hand.messh = ...
                        ...
                        'to straighten out the interpolant. (To see the Cubic Spline  ';...
                        'Interpolant for comparison, click on New.)'}, ...
                        messtitle, 'non-modal'};
            ...
            if ~isempty(cf)
                errordlg({['Something went wrong during input, as indicated', ...
                    ' by the following error message:'];laster.message},errortit)
                splinetool 'closefile'
            end
            return
        end %try
    ...
    case 'startfinish' % finish the rest of the gui figure:
        ...
        h2 = uimenu(h1,'Label','Export &Spline ...', ...
            ...
            'Tag','export_spline');
        h2 = uimenu(h1,'Label','&Generate Code ...', ...
        ...
        uimenu(h3,'Label','&Error Measure', 'Callback', ...
            'splinetool(''help'',''error'')')
        ...
        h2 = uimenu(h1,'Label','&Spline Commands', ...
            'Separator','on', ...
            'Callback', @helpOnSplineFunctions );
        ...
        sl = .032; rw = .15;
        lnpx = rw-sl; lnpy = mmpos(2); ww = lnpx-mmpos(1);
        wh = .045; ph = 3*wh;
    ...
    otherwise
        errordlg( ...
            sprintf( 'You provided just one input argument, the string ''%s''. If you supply input to SPLINETOOL, then it must be two vectors (of the same length).', x ), ...
            'Wrong input!','modal')
end % switch action
...
if ~isempty(answer)&&~isempty(answer{1})
    ...
    if ~isempty(answer{2}), xname = answer{2}; end
    if ~isf&&~isempty(answer{4}), yname = answer{4}; end
else   % take default data, but with  isf < 0
    isf = -1;
    [x,y] = titanium; xname = 'temperature'; yname = 'titanium property';
end %if ~isempty(answer)

function bad_edit(editfield,laster)
%BAD_EDIT report failure of change in edit field

warndlg( sprintf( ...
    'Evaluation of the expression you entered:\n''%s''\nproduced the following error message:\n\n%s', ...
    get(editfield,'String'), laster ), ...
...
if ~isvarname(name)
    uiwait(...
        warndlg( sprintf( 'The name ''%s'' is not a valid MATLAB variable name.', name ), ...
...
if ~isempty(findobj('Tag',name))
    uiwait(warndlg( sprintf( 'The name ''%s'' is already in use.', deblank(name) ), ...
        'Choose another name, please ...','modal'))
    % recover the current name and return it
    name = '';
end
...
if ~isequal(get(hand.figure,'Pointer'),'watch')
    set(hand.figure,'Pointer','watch'), drawnow
end
...
switch lineud.method
    case 1
        ...
        if any([2 3 4 9]==V)  % pick up the numerical endcondition information
            ...
            if ischar(output), set(hand.figure,'Pointer','arrow'), drawnow, return
            else
        ...
        switch V
            ...
            case 8 %  'Lagrange'
                lineud.cs = csape(x,y);
            case 9 %  'custom'
                tmp = get(hand.params(1,1),'String'); l = str2double(tmp(1));
                tmp = get(hand.params(1,2),'String'); r = str2double(tmp(1));
    ...
    case 2 % combine both cubic and quintic smoothing spline.
        ...
        switch tag
            ...
            case {'minusleft','editleft','plusleft'}% we specified the smoothing par ...
                ...
                output = convert(hand.params(2,1),shortstr(lineud.par,'',1),1);
                if ischar(output), set(hand.figure,'Pointer','arrow'), drawnow, return
            ...
            case {'minusclkr','editclkr','plusclkr'}
                ...
                if get(hand.data_etc,'Value')==3
                    ...
                    if V==length(x) % attempt to change the NaN entry
                        ...
                        set(hand.figure,'Pointer','arrow'), drawnow, return
                    end
                    temp = get(hand.params(1,5),'String');
                    output = convert(hand.params(2,5),eval(temp(V,:)));
                    if ischar(output), set(hand.figure,'Pointer','arrow'), drawnow, return
                        ...
                        if change<0
                            ...
                            if ich>1
                                if (changed-change)>abs(output)*1e-4
                                    warndlg( ...
                                        sprintf( 'In order to keep the roughness weight positive, the Spline Tool had to change the jump, %s, you proposed, to %s .', ...
            ...
            otherwise
                errordlg(sprintf('In smoothing, we have not yet covered the button:%s',tag), 'We must work harder!','modal')
        end %switch tag
        ...
        switch whichcall
            ...
            otherwise
                errordlg(sprintf('In smoothing, we have not yet covered the call:%s',whichcall), 'We must work harder!','modal')
    ...
    case 3  % least-squares approximation
        ...
        switch tag
            ...
            otherwise
                errordlg(sprintf('In least-squares, we have not yet covered the button:%s',tag), ...
                ...
                switch tag
                    case 'order'
                        ...
                        errmss = ['Foiled attempt to use too high an order for', ...
                            ' the given knots and data, as indicated by the following warning or error:'];
                    case 'del_item'
                        errmss = ['Deletion of that data point forced a change in the ',...
                            ...
                            'since, otherwise, there would have been trouble, as indicated by the following warning or error:'];
                    ...
                    otherwise
                        errmss = ['Foiled attempt to use a knot sequence that fails to',...
                            ...
                            ' given order wrto any subset of the data sites, as indicated by the following warning or error:'];
                        lineud.knots = fnbrk(lineud.cs,'knots');
                end
    ...
    case 4  % spline interpolation
        ...
        if moved
            ...
            if ischar(newcs)
                er=errordlg({['Foiled attempt to use a knot sequence that violates',...
...
depth = .8; width = .4; margo2 = (1-width)/2; inter = .01;
bh = min(.09,(depth/nargin)-inter); handles = zeros(1,nargin-1);
...
if (V==1&&x(1)~=xx)||(V==length(x)&&x(end)~=xx)
    h=warndlg('The Spline Tool won''t let you change the extreme data sites.', ...
        'Attempted change of interval of approximation','modal');
    uiwait(h)
elseif xx<x(1)||xx>x(end)
    h=warndlg(['The Spline Tool won''t let you move data sites beyond the', ...
        ' extreme ones.'],'Attempted change of interval of approximation','modal');
    uiwait(h)
elseif get(hand.method,'Value')==4 && any(x==xx)
    h=warndlg(['Since SPAPI would interpret repeated data sites',...
        ' as a request for osculatory interpolation, the Spline Tool',...
        ' won''t let you create repeated data sites now.'], ...
        'Repeated data sites', 'modal');
    uiwait(h)
...
function hand = new(hand, startmethod)
...
if nargin>1, set(hand.method, 'Value',startmethod), end
method(hand)
...
if ~isvarname(varname)
    uiwait(errordlg( ...
        sprintf('The proposed string\n''%s''\n is not a legal variable name.', varname ),...
        'Error','modal'))
    ok = 0; return
end
...
if evalin('base',['exist(''',varname,''',''var'')'])
    answer = questdlg( sprintf( ...
        'The variable ''%s'' already exists. Do you want to replace it?', varname ), ...
...
if isempty(answer) % ask for labels
    answer = inputdlg({'Give the x-label', ...
        ...
        ' a function, that function will be compared against', ...
...
if d<2, error(message('SPLINES:SPLINETOOL:wronginput')), end
...
if nargout==0
    ...
    for j=2:nargin
        ...
        if ischar(arg), symbol=arg;
            ...
            d = size(arg,2);
            if d~=1
                error(message('SPLINES:SPLINETOOL:wronginarg', num2str( j )))
            else
                linewidth = arg;
            end
        end
...
if isempty(get(hxlabel,'UserData'))
    set(hxlabel,'UserData',1, ...
        'String', sprintf( '%s (modified data)', get(hxlabel,'String') ) );
    if ~any(get(hand.viewmenu,'UserData'))
...
% For the 'Delete' item, we want to redirect the call to the SPLINETOOL legend
% toggle function
...
menu(cf,{'Choose some data to work on with the';
    ' SPLINE TOOL:'}, ...
...
if any(get(hand.viewmenu,'UserData'))
    ...
    tmp = get(hylabel,'String'); tmp1 = findstr(' ',tmp);
    set(hylabel,'String', ...
...
if viewud(ip)==1  % turn off the view
    ...
    switch ip
        case 1
            set(get(Axes2,'YLabel'),'String', sprintf( '1st Deriv. of %s', ufname ) );
        case 2
            set(get(Axes2,'YLabel'),'String', sprintf( '2nd Deriv. of %s', ufname ) );
        case 3
            set(get(Axes2,'YLabel'),'String', sprintf( 'Error in %s', ufname ) );
...
function helpOnSplineFunctions( ~, ~ )
% helpOnSplineFunctions -- Display help on Spline functions
%   helpOnSplineFunctions( src, evt )
mapfilename = fullfile( docroot, 'toolbox', 'curvefit', 'curvefit.map' );
helpview( mapfilename, 'spline_functions' );
