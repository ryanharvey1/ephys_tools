...
%   $Revision: 1.10.2.5 $  $Date: 2010/10/08 16:37:38 $
...
if length(x) ~= m
    error(message('curvefit:cfinterp1:invalidNumOfRows'))
end
if (m < 2)
    if isempty(u)
        v = [];
        ...
        error(message('curvefit:cfinterp1:notEnoughPoints'))
    end
end
if ~isreal(x)
    error(message('curvefit:cfinterp1:abscissaeNotReal'))
end
if ~isreal(u)
    error(message('curvefit:cfinterp1:interpNotReal'))
end
x = x(:);
h = diff(x);
if any(h < 0)
...
if any(h == 0)
    error(message('curvefit:cfinterp1:dataNotDistinct'));
end
siz = size(u);
u = u(:);
p = [];
...
otherwise
    ...
    otherwise
        ...
        error(message('curvefit:cfinterp1:invalidMethod'))
