classdef PredictionIntervalOptions
    ...
    %   $Revision: 1.1.6.4 $    $Date: 2010/10/08 16:36:59 $
    
    properties(Constant)
        % OBSERVATION is the value of INTERVAL correspondind to observation intervals
        OBSERVATION = 'Observation';
        % FUNCTIONAL is the value of INTERVAL correspondind to functional intervals
    ...
    methods(Access = 'private')
        function prop = findProperty( obj, prop )
            ...
            idx = strmatch( lower( prop ), lower( allProperties ) );
            if numel( idx ) == 1,
                prop = allProperties{idx};
            else
                ME = MException( 'curvefit:PredictionIntervals:InvalidProperty', ...
                    'The string ''%s'' does not match a unique property.', prop );
                throwAsCaller( ME );
            end
        end
    end
...
idx = strmatch( lower( value ), lower( validValues ) );
if numel( idx ) == 1,
    value = validValues{idx};
else
    ME = MException( 'curvefit:PredictionIntervalOptions:InvalidInterval', ...
        'INTERVAL must be either ''%s'' or ''%s''.', ...
        curvefit.PredictionIntervalOptions.OBSERVATION, ...
        curvefit.PredictionIntervalOptions.FUNCTIONAL);
    throwAsCaller( ME );
end
