...
%   $Revision: 1.1.6.10 $    $Date: 2011/05/09 00:39:51 $
...
if ~hasuimode(hFig,'sftoolgui.sfExcludeMode')
    ...
    set(hMode,'WindowButtonDownFcn',{@localWindowButtonDownFcn, ...
        hFitFigure, hMode});
    set(hMode,'WindowButtonUpFcn',{@localWindowButtonUpFcn, ...
        hFitFigure, hMode});
    set(hMode,'WindowButtonMotionFcn',{@localMotionFcn, hMode, hFitFigure});
else
...
uimenu(hMode.UIContextMenu, 'Label', xlate('Clear all exclusions'), ...
    'Callback', {@clearAllExclusions, hFitFigure});
end
...
if ~graphicsversion( hFig,'handlegraphics' )
    ...
    curr_units = hgconvertunits(hFig,[0 0 evt.CurrentPoint],...
...
if (localInBounds(hFitFigure.HSurfacePanel) || ...
    ...
    allHit = localHittest(hFitFigure.Handle,evt,'axes');
...
obj = sftoolgui.util.hittest( hFig, evd, varargin{:} );
end
