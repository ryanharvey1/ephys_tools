classdef Data < handle
    ...
    %   $Revision: 1.1.6.14 $   $Date: 2011/05/09 00:39:36 $
    ...
    methods
        ...
        function this = setVariableFromBaseWS(this, dataName, fieldName)
            ...
            if isempty(value)
                this.(fieldName) = sftoolgui.Variable();
            else
                this.(fieldName) = sftoolgui.Variable(dataName, value);
            end
            this.Name = createDataName(this);
        end
        ...
        function set.Version(this, version)
            ...
            if (version >= 3)
                ...
                error(message('curvefit:Data:IncompatibleVersion'));
            end
        end
    ...
    methods(Access = 'private')
        ...
        function setVariables(this, dataValues, dataNames)
            % setVariables creates sftoolgui.Variables with the cell array of
            % dataValues and dataNames supplied.
            FIELDS = {'X', 'Y', 'Z', 'W'};
            DEFAULT_NAMES = {'x', 'y', 'z', 'w'};
            
            % Valid lengths are 3 or 4. If 3, that means weights were not
            % specified
            for i = 1:length( dataValues )
                if ~isempty(dataValues{i})
                    if isempty(dataNames) || isempty(dataNames{i})
                        name = iGetUniqueVarName(DEFAULT_NAMES{i});
                    else
                        name = dataNames{i};
                    end
                    this.(FIELDS{i}) = sftoolgui.Variable(name, dataValues{i});
                else
                    this.(FIELDS{i}) = sftoolgui.Variable();
                end
            end
            % If weights were not specified, create an empty Variable
            if length(dataValues) == 3  % weights were not specified
        ...
        function messageStruct = incompatibleSizeMessage(this)
            ...
            elseif isPotentiallyMeshable( this )
                ...
                messageStruct.String = xlate('Data sizes are incompatible.');
        ...
        function messageStruct = NaNInfMessage(this)
            ...
            str = '';
            if hasNaN && hasInf
                str = xlate( 'Ignoring Inf and NaNs in data.' );
            elseif hasNaN
                str = xlate( 'Ignoring NaNs in data.' );
            elseif hasInf
                str = xlate( 'Ignoring Inf in data.' );
            end
        ...
        function messageStruct = complexMessage(this)
            ...
            if ~isreal( x ) || ~isreal( y ) || ~isreal( z ) || ~isreal( w )
                messageStruct.String = xlate( 'Using only the real component of complex data.' );
                messageStruct.Level = com.mathworks.toolbox.curvefit.surfacefitting.SFFitState.WARNING;
            end
        end
        ...
        function messageStruct = swappingMeshableVariablesMessage(this)
            ...
            if iIsXYZMeshable( this.Y, this.X, this.Z )  && ...
                ...
                messageStruct.String = ...
                    xlate('Using X data for rows and Y data for columns to match Z data matrix.');
                messageStruct.Level = com.mathworks.toolbox.curvefit.surfacefitting.SFFitState.WARNING;
            end
        end
        ...
        function messageString = getMeshableMismatchMessage(this)
            ...
            if this.W.Specified && ~isequal( this.Z.Size, this.W.Size )
                sizesDoNotMatchMessage = ...
                    xlate('Weights and Z sizes do not match.');
                convertingDataMessage = ...
                    xlate('Converting Weights and Z to column vectors.');
                messageString = sprintf( '%s%s%s\n%s', ...
                    sizesDoNotMatchMessage, ...
                    iSizeMessage(this.Z, xlate('Z data')), ...
                    iSizeMessage(this.W, xlate('Weights')), ...
                    convertingDataMessage);
            end
        end
        ...
        function messageString = getStandardMismatchMessage(this)
            ...
            if ~areAllVectors && ~areAllTheSameSize
                sizesDoNotMatchMessage = xlate('Data sizes do not match.');
                convertingDataMessage = ...
                    xlate('Converting all data to column vectors.');
                messageString = sprintf( '%s%s%s%s%s\n%s', ...
                    sizesDoNotMatchMessage, ...
                    iSizeMessage(this.X, xlate('X data')), ...
                    iSizeMessage(this.Y, xlate('Y data')), ...
                    iSizeMessage(this.Z, xlate('Z data')), ...
                    iSizeMessage(this.W, xlate('Weights')), ...
                    convertingDataMessage);
            end
        ...
        function messageStruct = nonDoubleMessage(this)
            ...
            messageStruct = [];
            if ~iIsDouble(this.X) ||  ~iIsDouble(this.Y) ||  ~iIsDouble(this.Z) || ~iIsDouble(this.W)
                messageStruct.String = xlate( 'Converting non-double values to double values.' );
                messageStruct.Level = com.mathworks.toolbox.curvefit.surfacefitting.SFFitState.WARNING;
            end
        end
    end
...
incompatibleMessage = xlate('Data are incompatible for curves.');
...
% INCOMPLETE.
theMessage.String = sprintf('Data are compatible for surface fitting if X is %s and Z is %s.', ...
...
% INCOMPLETE.
theMessage.String = sprintf('Data are compatible for surfaces if Z is %s.', ...
    iSizeString([numel(this.Y.Values), numel(this.X.Values)]));
theMessage.Level = com.mathworks.toolbox.curvefit.surfacefitting.SFFitState.INCOMPLETE;
end

function v = iValueFromBase( name, oldValue )
% iValueFrom Base returns a value from the base workspace from the given
% name.
if isempty(name)
    v = [];
else
