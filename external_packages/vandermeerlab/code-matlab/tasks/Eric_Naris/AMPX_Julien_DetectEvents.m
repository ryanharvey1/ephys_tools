...
PARAM_detect_thr = [0.95 .95 0.98 .98]; % threshold for event detection: 95th percentile of (amplitude) envelope
...
PARAM_chew_thr = 2; % z-score of chew band envelope must be smaller than this, default 0.25
...
for iFreq = 1:length(PARAM_f_label)
    ...
    if debug2
        ...
        for ii = 1:length(h)
            ...
            print(h(ii), ['C:\temp\Gamma_detection\' num2str(cfg_plot.fname) '\' PARAM_f_label{iFreq} '_' num2str(ii) '.png'])
        end
        close all;
