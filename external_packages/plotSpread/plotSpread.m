    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues,m,'-','Color','k','MarkerSize',100);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues,md,'-','MarkerSize',100);
            end
            if opt.showMM == 4
                mh = plot(ah,opt.xValues,m,'+k','Color','k','MarkerSize',100);
        ...
        case 'flipped'
            if any(opt.showMM==[1,2])
                mh = plot(ah,m,opt.xValues,'+r','Color','r','MarkerSize',20,'MarkerFaceColor','r','MarkerEdgeColor','r');
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,md,opt.xValues,'+k','MarkerSize',20,'MarkerFaceColor','k','MarkerEdgeColor','k');
            end
            if opt.showMM == 4
