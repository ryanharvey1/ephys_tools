...
% load eeg file
path = 'F:\Users\BClarkLab\Desktop\Ryan\Test_session\2016-10-27_18-10-02';
...
for x=1:length(Timestamps)
    for j=2:factor
        EEGtsUpSampled(j,x)=Timestamps(x)+TSdiffIncrements(x)*(j-1);
    end
end
...
EEGthetaData = eegfilt(EEG_DownSampledData,NewSFreq,6,10);
...
[filepath, filename] = fileparts(eegfile{i});
    save([filepath filesep filename '_ThetaPower.mat']);
