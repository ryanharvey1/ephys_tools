...
ts_spike = interp1(data_video_smoothvelfilt(:,1), data_video_smoothvelfilt(:,1), s1, 'nearest');
x_spike = interp1(data_video_smoothvelfilt(:,1), data_video_smoothvelfilt(:,2), s1, 'nearest');
y_spike = interp1(data_video_smoothvelfilt(:,1), data_video_smoothvelfilt(:,3), s1, 'nearest');
angle_spike = interp1(data_video_smoothvelfilt(:,1), data_video_smoothvelfilt(:,4), s1, 'nearest');
% 
% create array for binning data
nBins = 26; % adjusted so the bins are ~5x5cm as in Koenig et al 2011
MinY = min(data_video_smoothvelfilt(:,2));
MaxY = max(data_video_smoothvelfilt(:,2));
MinX = min(data_video_smoothvelfilt(:,3));
MaxX = max(data_video_smoothvelfilt(:,3));
...
figure (1)
plot(data_video_smoothvelfilt(:,2), data_video_smoothvelfilt(:,3), 'LineWidth', 1, 'color', 'k');
hold on;
scatter(x_spike, y_spike, 45, 'filled', 'r');
