...
for j=1:size(param_idx,1)
  ...
  %% Maze Parameters;
        ...
        % FIND INDEX FOR STOPPING (<3cm/second for 1 frame)
...
%         plot(params.transcoords{j}(:,1),params.transcoords{j}(:,2),'.k');hold on
        for ii=1:length(start)
            ...
            params.stops{j}=num2cell(motionless);
...
%             plot(params.transcoords{j}(start(ii):ends(ii),1),params.transcoords{j}(start(ii):ends(ii),2),'.k');hold on
        ...
        for num=1:length(motionless)
            params.proximity{j}(1,num)=sqrt((params.HBcenter{j}(1,1)-motionless{1,num}(1,1))^2+(params.HBcenter{j}(1,2)-motionless{1,num}(1,2))^2);
        end
