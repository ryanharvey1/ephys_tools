...
load('params_V2.mat') %%loads table containing the raw coords and xMax/Min and yMax/Min
...
for j=1:size(param_idx,1)
  ...
  %% Maze Parameters;
        ...
        %find
        %
        %         %Calculate circuity of path segments
        ...
        outsideWall=createZones([0,0],params.dia(j),'type','annulus','fig',1);
        ...
        [start,ends,ngroups]=findgroups(params.stopIdx{j});
        ...
        [start,ends,ngroups]=findgroups(params.runIdx{j});
        
        figure;
%         plot(params.transcoords{j}(:,1),params.transcoords{j}(:,2),'.k');hold on
        for ii=1:length(start)
            motion{ii}=[params.transcoords{j}(start(ii):ends(ii),1),params.transcoords{j}(start(ii):ends(ii),2)];
            plot(params.transcoords{j}(start(ii):ends(ii),1),params.transcoords{j}(start(ii):ends(ii),2),'.k');hold on
        end
